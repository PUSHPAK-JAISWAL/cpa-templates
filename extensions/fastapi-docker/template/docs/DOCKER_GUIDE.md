...
The **fastapi-docker** extension adds a reproducible container workflow for FastAPI (or other uv-based Python API) projects. Compose files follow Create-Node-App naming: **`compose.yml`** / **`compose.prod.yml`** (not `docker-compose.yml`).

Use it when you want local containers without installing Python on the host, or when you ship the API as an image. Pair with **postgres** when you need a database service.
...
| Postgres hostname errors | Using `localhost` inside compose | Use service name `db` when stacked with `postgres` |
